...
class CasadiMPCNode(Node):
    def __init__(self):
        ...
        self.goal_state = ca.DM([2, 2, 0])
        self.get_logger().info('Goal set to: x=2, y=2, theta=0')
    ...
    # Ensure current state data is valid before proceeding
        if self.current_state is None:
            self.get_logger().info("Waiting for current state data...")
            return

    # Initial guess for the optimization
        x0 = np.zeros((3, self.horizon + 1))
        x0[:, 0] = np.array(self.current_state.toarray()).flatten()
    ...
    # Solve the optimization problem
        res = self.solver(
            ...
            lbg=[0] * (self.horizon * 3 + 3),
            ubg=[0] * (self.horizon * 3 + 3),
